...
export default function FichaCampoPrint({ clientId, campoId }: { clientId: string; campoId: string }) {
  const goBack = useSmartBack(`/dashboard/cliente/${clientId}/campo/${campoId}`);
  ...
  if (error || !ficha || ficha.campos.length === 0) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-white">
        <p className="text-gray-500 text-sm">No se pudo cargar la información del campo.</p>
      </div>
    );
  }

  const campo = ficha.campos[0];
  ...
  return (
    <div className="bg-white min-h-screen">
      ...
      <div className="ficha-print max-w-4xl mx-auto px-6 py-6 print:px-0 print:py-0">
        ...
        {/* Encabezado */}
        <div className="bg-linear-to-r from-blue-600 to-blue-700 text-white p-6 print:p-4 rounded-lg mb-6 print:mb-4">
          <div className="flex items-start justify-between">
            <div>
              <p className="text-blue-200 text-sm font-semibold uppercase tracking-wider mb-0.5">
                {ficha.clientName}
              </p>
              <h1 className="text-3xl print:text-2xl font-bold mb-1">Ficha Técnica — Campo</h1>
              <p className="text-blue-100 text-sm">
                Generado el {formatDate(generatedAt)} a las {formatTime(generatedAt)}
              </p>
            </div>
            <div className="text-right">
              <p className="text-2xl font-bold">{campo.name}</p>
              <p className={`text-sm mt-1 ${campo.active ? 'text-green-300' : 'text-yellow-300'}`}>
                {campo.active ? '✓ Campo activo' : '⚠ Campo inactivo'}
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="mb-6 flex flex-wrap gap-4 items-start">
          ...
          {campo.totalSector > 0 && <SectorBar total={campo.totalSector} kml={campo.kmlSector} />}
        </div>
        ...
        {campo.talgilDevices.length === 0 &&
         ...
         campo.davisDevices.length === 0 &&
         campo.florapulseDevices.length === 0 && (
          <p className="text-sm text-gray-400 italic text-center py-8">Sin dispositivos registrados en este campo</p>
        )}
        ...
        <footer className="mt-10 pt-4 border-t border-gray-200 flex items-center justify-between text-xs text-gray-400">
          <span>Olive Ops — Ficha de campo</span>
    ...
    <div className="bg-blue-50 border border-blue-200 rounded-lg p-3 print:p-2">
      <div className="flex items-start justify-between mb-2">
        ...
        <div className="flex flex-wrap gap-1 justify-end">
          {modules.map(m => (
            <span key={m.label} className={`text-xs px-1.5 py-0.5 rounded border ${
              m.active
...
function SectorBar({ total, kml }: { total: number; kml: number }) {
  ...
  const sin = total - kml;
  return (
    <div className="ml-auto text-xs text-gray-700 min-w-[180px]">
